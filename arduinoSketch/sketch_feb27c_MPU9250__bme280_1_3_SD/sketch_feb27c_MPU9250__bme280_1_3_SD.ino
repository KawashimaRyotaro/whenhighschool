...
String st_text="", st_csv="";
String dataString_text_h="",dataString_text_l="",dataString_csv="";
...
void setup() {
    ...
    uint8_t t_sb = 0;               //Tstandby 1000ms
 ...
 // MadgwickFilter.begin(30);
  ...
  st_text="datalog"+String(0)+".csv";
  //st_csv="datalog"+String(0)+".csv";
  
  for(int i=1;SD.exists(st_text)==1;i++){
    if(i>=10){
      st_text="datal"+String(i-10)+".csv";
    }else{
      st_text="datalog"+String(i)+".csv";
    }
  }
  
  /*for(int i=1;SD.exists(st_csv);i++){
    st_csv="datalog"+String(i)+".csv";
  }
  File dataFile_1= SD.open(st_csv, FILE_WRITE);

  dataFile_1.println(",ax,ay,az,gx,gy,gz,mx,my,mz,press,temp,hum");
...
void loop() {
 ...
 //MadgwickFilter.update(gx1, gy1, gz1, ax1, ay1, az1, magX2, magY2, magZ2);

  /*float ROLL  = MadgwickFilter.getRoll();
  float PITCH = MadgwickFilter.getPitch();*/

 // double h=((pow(1009.2/press_act,1/5.257)-1)*(temp_act+273.15))/0.0065;
  dataString_text_l=String(press_act)+","+String(temp_act)+","+String(hum_act);
  //dataString_csv+=","+String(ax1)+","+String(ay1)+","+String(az1)+","+String(gx1)+","+String(gy1)+","+String(gz1)+","+String(magX2)+","+String(magY2)+","+String(magZ2)+","+String(press_act)+","+String(temp_act)+","+String(hum_act);
  //Serial.println("csvdata:"+dataString_csv);

  File dataFile= SD.open(st_text, FILE_WRITE);
  
 if(dataFile){
  Serial.print(st_text+":"+dataString_text_h);
  Serial.println(dataString_text_l);
  dataFile.println(dataString_text_h+dataString_text_l);
  dataFile.close();
 }else{
 }
 ...
 if(dataFile_1){
  dataFile_1.println(dataString_csv);
  dataFile_1.close();
 }else{
 }*/
 dataString_text_h="";
